...
let presentationSched = [];

let tempPresentation = {};

////
let dayofWeek = ['mon', 'tues', 'wed'];
let dayCount = 0;
let timeArr = ['AM', 'PM'];
let timeCount = 0;
let seshNum = 0;
let maxStudentsPerRoomPerSesh = 12;
let numberOfClassrooms = allocatedProjects.length / 72;
let classroomNum = -1;
let presID = 0;
///
...
for (let i = 0; i < allocatedProjects.length; i++) {
  if (i % 12 == 0) {
    tempPresentation.presentation_ID = presID;
    presID++;
    tempPresentation.classroomNumber = classroomNum % numberOfClassrooms;
    classroomNum++;

    tempPresentation.time = timeArr[timeCount % 2];
    if (classroomNum % numberOfClassrooms == 0) {
      timeCount++;
    }
    tempPresentation.day = dayofWeek[dayCount % 3];
    if (timeCount % 2 == 0) {
      dayCount++;
      classroomNum = 0;
    }
    ...
    tempPresentation.chair_ID = -1;
    tempPresentation.second_marker_ID = -1;

    presentationSched.push(tempPresentation);
  }
  allocatedProjects[i].presentation_ID = presID;
}

console.log(presentationSched);
