...
let allocatedProjects = [
  ...
  let tempPres = {}; // Used to build schema before pusing onto presSched
  
  //Data to be built according to schema 
  let presID = 1; //Unique presID = 1,2,3 ...
  ...
  let classroomNum = 1; //for MVP, classroomNum = 1,2,3 ... substitues room code
  ...
  */
  ...
  for (let i = 0; i < allocatedProjects.length; i++) { //loop through all projects
    //students in the same class and same sesh should have same presID, i.e. unique seshID for every 12 students
    if (i % studentsPerRoomPerSesh == 0) { //at the start of a new batch of 12 students
      //building according to schema
      tempPres.presentationID = presID; //Unique presentation ID
      tempPres.classroomNumber = classroomNum;

      //idea: assign all in tempPres here, update all value based on current presID

      if(presID % 2 == 0){
        classroomNum++;
      }
      presID++; //increment after assigning to ensure uniqueness
  
      tempPres.time = seshArr[timeCount % 2];
      if (classroomNum % numberOfClassrooms == 0) {
        timeCount++;
      }
      tempPres.day = dayofWeek[dayCount % 3];
      if (timeCount % 2 == 0) {
        dayCount++;
        classroomNum = 0;
      }
      ...
      tempPres.chairID = -1; 
      //Problem: how to allocate second marker
      //Problem: Second marker data duplicated?
