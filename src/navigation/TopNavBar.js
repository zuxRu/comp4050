...
export default function TopNav({isStudent, isAcademic}) {
  const title = () => {
    if(isStudent){
      return 'Student';
    } else {
    ...
    return (
        <nav className={`navbar navbar-expand-lg ${styles.customNavBar} fixed-top`}>
           <Link className={styles.home} to="/dashboard">
                    <Home />
            </Link>
          <a className={`navbar-brand ${styles.centerItems}`} href="#">{title()}</a>
